...
pub fn home() -> Html {
    ...
    let faux_code = html! {
        <>
            ...
            <span>{"        "}<span class="string">{"println!(\"Hello, I'm "}<span class="highlight-name">{"Daimler Garay"}</span>{", a passionate "}<span class="highlight-role">{"DevOps engineer"}</span>{" and "}<span class="highlight-role">{"Rustacean"}</span>{"!\");"}</span></span><br/>
        ...
        <>
            ...
            <main class="flex flex-col items-center justify-center p-4 w-full text-center md:text-left">
                <div class="m-4 p-4 bg-opacity-80 bg-rich-blue rounded-full shadow-lg transition duration-700 ease-in-out hover:rotate-12 hover:bg-purple-600">
                    <img src="/home/dgaray/Pictures/guts.jpg" class="rounded-full border-4 border-secondary shadow-xl w-64 h-64 transition-transform duration-500 ease-in-out hover:scale-125" alt="Profile Image" />
                </div>
